...
export function ProjectPage() {
  ...
  const {
    projects, agents, threads, events, instructions, orgGraph, connected,
    fetchProjects, loadProject, fetchAgents, fetchOrgGraph, fetchThreads,
    ...
    <div style={{ display: "flex", height: "100vh", background: "#0f172a", color: "#f1f5f9", fontFamily: "Inter, system-ui, sans-serif" }}>
      <aside style={{ width: 220, background: "#1e293b", borderRight: "1px solid #334155", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "16px", borderBottom: "1px solid #334155" }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9" }}>{project?.name ?? projectId}</div>
          {project?.description && (
            <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>{project.description}</div>
          )}
          <button
            onClick={() => navigate("/")}
      ...
      <main style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          ...
          {view === "execution" && (
            <ExecutionView projectId={projectId!} instructions={instructions} agents={agents} />
          )}
      ...
      {showCreateAgent && (
        <CreateAgentModal
          ...
          onCreated={() => { fetchAgents(projectId!); fetchOrgGraph(projectId!); }}
        />
      )}
      {showCreateThread && (
        <CreateThreadModal
          ...
          onCreated={() => fetchThreads(projectId!)}
        />
      )}
      {selectedThread && (
        <ThreadTimeline
